...
class TemplateLoader {
     ...
     */
    testTemplate(template, variables = {}) {
        ...
        try {
            ...
            const defaults = this.getDefaults('placeholders');
            ...
            while ((match = variablePattern.exec(template)) !== null) {
                ...
                if (allVariables.hasOwnProperty(variableName)) {
                    foundVariables.push(variableName);
                    result = result.replace(match[0], allVariables[variableName]);
                } else {
            ...
            return {
                ...
                usedDefaults: Object.keys(defaults).filter(key => 
                    foundVariables.includes(key) && !variables.hasOwnProperty(key)
                )
            };
    ...
    getStatistics() {
        ...
        const stats = {
            loaded: true,
            platforms: Object.keys(this.templates.servers).length,
            cacheSize: this.cache.size,
            templates: {}
        };

        for (const [platform, platformData] of Object.entries(this.templates.servers)) {
            stats.templates[platform] = {};
            
            for (const [serverName, serverData] of Object.entries(platformData)) {
                let templateCount = 0;
                ...
                countTemplates(serverData);
                stats.templates[platform][serverName] = templateCount;
            }
        }
