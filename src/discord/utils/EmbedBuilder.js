...
class EmbedBuilder {
    ...
    createStatsEmbed(stats) {
        const embed = new DiscordEmbedBuilder()
            .setColor(this.colors.system)
            .setTitle(`${this.emojis.system} System Statistics`)
            .setTimestamp();

        // Add various statistics fields
        if (stats.minecraft) {
            embed.addFields({
                name: '🎮 Minecraft',
                value: [
                    `Connections: ${stats.minecraft.connectionsActive}/${stats.minecraft.connectionsTotal}`,
                    `Messages: ${stats.minecraft.messagesProcessed || 0}`,
                    `Events: ${stats.minecraft.eventsProcessed || 0}`
                ].join('\n'),
                inline: true
            });
        }
        ...
        if (stats.discord) {
            embed.addFields({
                name: '💬 Discord',
                value: [
                    `Messages Sent: ${stats.discord.messagesSent || 0}`,
                    `Webhooks Used: ${stats.discord.webhooksSent || 0}`,
                    `Embeds Sent: ${stats.discord.embedsSent || 0}`
                ].join('\n'),
                inline: true
            });
        }
        ...
        if (stats.interGuild) {
            embed.addFields({
                name: '🌉 Inter-Guild',
                value: [
                    `Messages: ${stats.interGuild.messagesProcessed || 0}`,
                    `Events: ${stats.interGuild.eventsProcessed || 0}`,
                    `Queue Size: ${stats.interGuild.queueSize || 0}`
                ].join('\n'),
                inline: true
            });
        }
        ...
        if (stats.system) {
            const uptimeHours = stats.system.uptime ? Math.round(stats.system.uptime / (1000 * 60 * 60) * 100) / 100 : 0;
            embed.addFields({
                name: '⚙️ System',
                value: [
                    `Uptime: ${uptimeHours}h`,
                    `Memory: ${stats.system.memory || 'N/A'}`,
                    `Errors: ${stats.system.errors || 0}`
                ].join('\n'),
                inline: true
            });
        }
